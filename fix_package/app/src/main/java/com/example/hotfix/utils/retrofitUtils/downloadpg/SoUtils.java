...
public class SoUtils {
    ...
    public static void checkOrDownloadSo(String totalUrl) {
        ...
        if (savePathFile.exists() && countLength == savePathLenth && readLength == savePathLenth) {
            ...
            createNewNativeDir(MyApplicationPlug.instance, downInfo.getUrl());
            // 加载so库;
            ...
            MyApplicationPlug.isSoLibDownloading = false;
        } else {
            // 开启So库的下载
            MyApplicationPlug.isSoLibDownloading = true;
            ...
            MainActivity.mainActivityInstance.presenterImpl.requestDownLoadFromWeb(totalUrl,
                    (long progress, long total, boolean done, DownInfo downingInfo) -> {
                        ...
                        if (done) { // 下载完毕，加载so库
                            ...
                            createNewNativeDir(MyApplicationPlug.instance, downingInfo.getUrl());
                            // 加载so库;
    ...
    public static void checkCpuType() {
        String cpuOS = SystemPropertiesProxy.get(MyApplicationPlug.instance, "ro.product.cpu.abi");
