...
public class DownInfo {
    ...
    public static DownInfo getInstance(String totalUrl, int requestType, String fileName) {
        if (requestType == SO_LIBRARY) {
            Toast.makeText(MyApplication.instance, "So库动态加载不可以自定义文件名！", Toast.LENGTH_LONG);
            return null;
        }
    ...
    public static DownInfo getInstance(String totalUrl, int requestType) {
        ...
        if (!downInfoHashMap.containsKey(totalUrl)) {
            String soDownloadInfo = SharePerferenceUtil.getVaule(MyApplication.instance, "file_down_load", totalUrl);
            if (!soDownloadInfo.equals("")) {
                try {
                    ...
                    // 清空本地缓存数据
                    SharePerferenceUtil.saveValue(MyApplication.instance, "file_down_load", totalUrl, "");
                    if (totalUrl.startsWith("http://") || totalUrl.startsWith("https://")) {
                        instance = new DownInfo(totalUrl);
                        instance.downLoadFileType = requestType;
                        downInfoHashMap.put(totalUrl, instance);
                    } else {
                        Toast.makeText(MyApplication.instance, "参数必须填写完整的下载地址！", Toast.LENGTH_LONG);
                        return null;
                    }
                }
            } else {
    ...
    public void saveInstance(String totalUrl) {
        ...
        if (downInfo != null) {
            ...
            // 存储json
            SharePerferenceUtil.saveValue(MyApplication.instance, "file_down_load", totalUrl, instanceJson);
        } else {
            // 存储json
            SharePerferenceUtil.saveValue(MyApplication.instance, "file_down_load", totalUrl, "");
        }
    ...
    public void setUrl(String url) {
        ...
        if (downLoadFileType == SO_LIBRARY) {
            ...
            savePath = MyApplication.instance.getFilesDir().getPath() + "/download/" + rawFileName.substring(cutIndex + 1, rawFileName.length());
        } else if (downLoadFileType == APK_FILE) {
            if (fileName.equals(""))
                savePath = MyApplication.instance.getFilesDir().getPath() + "/download/" + System.currentTimeMillis() + ".apk";
            else
                savePath = MyApplication.instance.getFilesDir().getPath() + "/download/" + fileName + ".apk";
        }
    }
