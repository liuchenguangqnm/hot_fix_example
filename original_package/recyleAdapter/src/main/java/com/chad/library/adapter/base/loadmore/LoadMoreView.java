...
public abstract class LoadMoreView {
    ...
    private void visibleLoading(BaseViewHolder holder, boolean visible) {
        View loadingView = holder.getView(getLoadingViewId());
        if (visible)
    ...
    private void visibleLoadFail(BaseViewHolder holder, boolean visible) {
        View loadingFailView = holder.getView(getLoadFailViewId());
        if (visible)
            loadingFailView.setVisibility(View.VISIBLE);
        else
            loadingFailView.setVisibility(View.INVISIBLE);
    }

    private void visibleLoadEnd(BaseViewHolder holder, boolean visible) {
        View loadingEndView = holder.getView(getLoadEndViewId());
        if (visible)
            loadingEndView.setVisibility(View.VISIBLE);
        else
            loadingEndView.setVisibility(View.INVISIBLE);
    }
